...
from dicty.dicty import DictyDict
...
def test_batched_getitem(list_of_dicts):
	d = dt.Dicty(list_of_dicts)
	assert d['a'] == dt.Dicty([dd['a'] for dd in d]) 

def test_batched_setitem(list_of_dicts):
	d = dt.Dicty(list_of_dicts)
	d['a'] = 10
...
class TestNumpy:
	...
	def test_apply(self, dicts):
		print("np.sum: {}".format(dt.apply(np.sum, dicts[0], axis=0)))
		print("np.dot: {}".format(dt.apply(np.dot, dicts[0], dt.apply(np.transpose, dicts[1]))))
